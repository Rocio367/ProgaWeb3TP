...
namespace Repositorios
{
    public class RepositorioPedido : IRepositorioPedido
    {
        ...
        public List<Pedido> ObtenerPedidosSinFiltro()
        {
            ...
            return _context.Pedidos.Include(e => e.IdClienteNavigation).Include(e => e.ModificadoPorNavigation).Include(e => e.IdEstadoNavigation).Where(a => a.FechaBorrado == null && now.Month - a.FechaCreacion.Month <= 2 && a.IdEstado==1).ToList();
        ...
        public List<Pedido> ObtenerPedidosConFiltro(int? id_cliente, int? id_estado, Boolean eliminados = true, Boolean ult_meses = true)
        {
            ...
            if (ult_meses)
            {
                if (resultadosFiltro.Count() == 0)
                {
                    ...
                    resultadosFiltro = todos.Where(e => now.Month - e.FechaCreacion.Month <= 2).ToList();
                }
                else
                {
                    resultadosFiltro = resultadosFiltro.Where(e => now.Month - e.FechaCreacion.Month <= 2).ToList();
                }
